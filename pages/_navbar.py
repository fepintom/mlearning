...
def navbar(active: int = -1):
    ...
    if ds_title and active >= 2:
        link = f'<a href="{ds_url}" target="_blank" style="color:var(--gold);text-decoration:none;font-size:11px">Ver en Kaggle ↗</a>' if ds_url else ""
        st.markdown(f"""
        <div style="background:var(--navy-800);border:1px solid var(--navy-600);
                    border-left:4px solid var(--gold);border-radius:10px;
                    padding:12px 16px;margin-bottom:4px">
          <div style="font-size:12px;font-weight:700;color:var(--gold);margin-bottom:6px">
            Dataset activo: {ds_title} &nbsp; {link}
          </div>
        </div>
        """, unsafe_allow_html=True)
        if ds_desc:
            with st.expander("Ver descripción completa del dataset", expanded=False):
                st.markdown(ds_desc.replace("\n", "\n\n"))
